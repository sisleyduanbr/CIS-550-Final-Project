...
export default function LazyTable({ route, columns, defaultPageSize, rowsPerPageOptions }) {
  ...
  const handleChangePageSize = (e) => {
    ...
    // DONE (TASK 18): set pageSize state variable and reset the current page to 1
      ...
      <Table>
        ...
        <TableBody>
          {data.map((row, idx) =>
            <TableRow key={idx}>
              {
                // DONE (TASK 19): the next 3 lines of code render only the first column. Modify this with a map statement to render all columns.
